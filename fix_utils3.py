...
with open('utils.ts', 'r') as f:
    content = f.read()

# I will find the function generateLocalFallbackAnalysis and replace everything inside it.
# Actually it's easier to just append a new function and rename the old one, but let's just do a clean string replacement.

start_idx = content.find('export const generateLocalFallbackAnalysis = (')
if start_idx != -1:
    end_idx = content.find('export function cn(', start_idx)
    
    new_func = """export const generateLocalFallbackAnalysis = (
